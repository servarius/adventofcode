...
def prepare_pwd_policies(passwords: str):
    """ Splits policies and prepares it for check_pwd_by_policy """

    rules = []
    policies = passwords.splitlines()

    for policy in policies:
        min_max, letter, password = policy.split()
        min, max = min_max.split('-')
        rules.append((int(min), int(max), letter[0], password))

    return rules

def check_pwd_by_policy(min: int, max: int, symbol: str, password) -> bool:
    """
    Checks whether password sticks to password policy requirements:

    :param min: minimal count of symbol
    :param max: maximum count of symbol
    :param symbol: symbol to check
    :param password: password to check
    :return: True if password is correct
    """

    occurrencies = password.count(symbol)
    if min <= occurrencies <= max:
        return True
    return False
...
def count_correct_passwords(passwords):
    """ Return how many passwords are stick to policies """
    rules = prepare_pwd_policies(passwords)
    checks = [check_pwd_by_policy(min, max, symbol, password) for min, max, symbol, password in rules]
    return sum(checks)

# second part


def check_pwd_by_position_policy(first_sym_pos: int, second_sym_pos: int, symbol: str, password) -> bool:
    """
    Checks whether password sticks to password policy requirements:

    :param first_sym_pos: minimal count of symbol
    :param second_sym_pos: maximum count of symbol
    :param symbol: symbol to check
    :param password: password to check
    :return: True if password is correct
    """
    if len(password) < second_sym_pos or len(password) < first_sym_pos:
        return False

    occurences = password[first_sym_pos-1] + password[second_sym_pos-1]

    return True if occurences.count(symbol) == 1 else False


def count_correct_passwords_by_pos(passwords):
    """ Return how many passwords are stick to position-typed policies """
    rules = prepare_pwd_policies(passwords)
    checks = [check_pwd_by_position_policy(first_sym_pos, second_sym_pos, symbol, password)
              for first_sym_pos, second_sym_pos, symbol, password in rules]
    return sum(checks)

if __name__ == '__main__':
    print(count_correct_passwords(passwords))
    print(count_correct_passwords_by_pos(passwords))
